...
def compileOneTime( studentID, output ) :
	#make dir
	try:
	...
	returnOutput = subprocess.check_output("docker build -t "+studentID+"image"+" . --no-cache", shell=True)
	...
	#find image
	imageIDwithName = subprocess.check_output("docker images --format \"{{.ID}}: {{.Repository}}\" | grep "+studentID+"image | cut -d\" \" -f2", shell=True)
	imageIDwithName=imageIDwithName[:-1] #"...\n"
	imageIDwithName=imageIDwithName.decode("utf-8")
	#print(imageIDwithName)

	#kill image
	subprocess.check_output("docker rmi "+imageIDwithName, shell=True)
